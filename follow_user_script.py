...
def format_command(title, cmd, args):
    """
...
def parseCommandLine(args):
    ...
    if args.sim:
        ...
        commands_to_run.append(format_command("AMR Simulation", sim_launch_cmd, args))
    ...
    if args.task_coordinator or args.mock:
        ...
        commands_to_run.append(format_command("Task Coordinator", task_coordinator_launch_cmd, args))

    if args.mock:
        # Launch perception and control manager
        perception_manager_cmd = 'ros2 run perception_control_manager perception_control_manager_node.py'
        commands_to_run.append(format_command("Perception Manager", perception_manager_cmd, args))
        
        # Launch mock HTTP API server
        mock_http_server_cmd = 'python3 mock_api_server.py'
        commands_to_run.append(format_command("Mock HTTP API Server", mock_http_server_cmd, args))

    if args.test:
        test_navigation_cmd = 'ros2 run navigation test_navigation.py'
        commands_to_run.append(format_command("Test Navigation", test_navigation_cmd, args))
...
def execute_commands(commands_list, debug_mode, sleep_time=1.0):
    ...
    for command in commands_list:
        ...
        time.sleep(sleep_time) # Give time for the new terminal to open

...
class GuidanceActionServer(Node):
    ...
    def _get_human_distance(self, human_relative_pose) -> float:
        ...
        if human_relative_pose is None:
            self.get_logger().debug(f"Human relative pose is not exist.")
            return float('inf')
    ...
    async def _adjust_speed_and_handle_lost(self, human_distance: float):
        ...
        if (self._last_speed_change_time is None or 
            ...
            if human_distance < self.normal_distance_min:
                ...
                self._is_human_lost = False
                self._human_lost_start_time = None
                self.get_logger().info(f"TooClose  ---> Distance:{human_distance}, moving_speed:{new_max_moving_speed}, angular_speed:{new_max_angular_speed}")
                ...
                if human_distance > self.lost_distance_threshold:
                    ...
                    status = "Lost"
                    self.get_logger().warn(f"Lost      ---> Distance:{human_distance}, moving_speed:{new_max_moving_speed}, angular_speed:{new_max_angular_speed}")
                    ...
                    self._is_human_lost = False
                    self._human_lost_start_time = None
                    self.get_logger().info(f"Lagging   ---> Distance:{human_distance}, moving_speed:{new_max_moving_speed}, angular_speed:{new_max_angular_speed}")
            else:
                # Normal following
                ...
                self._is_human_lost = False
                self._human_lost_start_time = None
                self.get_logger().info(f"Following ---> Distance:{human_distance}, moving_speed:{new_max_moving_speed}, angular_speed:{new_max_angular_speed}")
    ...
    async def execute_callback(self, goal_handle):
        ...
        while rclpy.ok():
            ...
            if guiding_stage == "ABORTING":
                self.get_logger().info("Goal aborted due to human lost!")
                await self.publish_cancel()
                goal_handle.abort()
                result.success = False
                return result
            ...
            if self.current_pose and not self.remaining_targets:
                if self._is_goal_reached(self.current_pose, target_pose):
                    # Reset to default speed before aborting
                    self.set_max_speed(self._speed_ratio * self.max_moving_speed, 
                                       self._speed_ratio * self.max_angular_speed)
